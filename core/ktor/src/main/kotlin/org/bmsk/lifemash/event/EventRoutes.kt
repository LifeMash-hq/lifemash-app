    ...
    authenticate("auth-jwt") {
        route("/events/{eventId}") {
            get {
                ...
                call.respond(eventService.getEventDetail(userId, eventId))
            }
            post("/join") {
                val userId = call.principal<JWTPrincipal>()!!.userId()
                val eventId = call.parameters["eventId"]!!
                val isJoined = eventService.toggleJoin(userId, eventId)
                call.respond(org.bmsk.lifemash.model.calendar.ToggleJoinResponse(isJoined))
            }
        }
