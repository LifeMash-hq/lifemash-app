...
class MomentService(
    ...
    fun getById(momentId: Uuid): MomentDto {
        return momentRepository.findById(momentId)
            ?: throw NotFoundException("순간을 찾을 수 없습니다")
    }

    fun update(momentId: Uuid, requesterId: Uuid, request: UpdateMomentRequest): MomentDto {
        val moment = momentRepository.findById(momentId)
            ?: throw NotFoundException("순간을 찾을 수 없습니다")
        if (moment.authorId != requesterId.toString()) {
            throw ForbiddenException("본인의 순간만 수정할 수 있습니다")
        }
        if (request.visibility != null && request.visibility !in VALID_VISIBILITY) {
            throw BadRequestException("공개범위는 public, followers, private 중 하나여야 합니다")
        }
        val mediaList = request.media
        if (mediaList != null && mediaList.size > 10) {
            throw BadRequestException("미디어는 최대 10개까지 첨부할 수 있습니다")
        }
        return momentRepository.update(momentId, request.caption, request.visibility, mediaList)
            ?: throw NotFoundException("순간을 찾을 수 없습니다")
    }
