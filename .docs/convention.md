...
## 7. 상태 변경은 public 함수에서만

`_uiState` 변경은 ViewModel의 public 함수에서만 허용한다. private 함수는 데이터를 반환만 하고 상태를 변경하지 않는다.

자세한 내용은 [상태 설계 원칙 — 6.5](state-design-principles.md#65-상태-변경은-public-함수에서만) 참조.
